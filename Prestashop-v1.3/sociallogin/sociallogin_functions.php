...
function storeAndLogin($user_profile_data, $rand=''){
	...
	$query= "INSERT into "._DB_PREFIX_."customer (`id_gender`,`id_default_group`,`firstname`,`lastname`,`email`,`passwd`,`last_passwd_gen`,`birthday`,`active`,`date_add`,`date_upd`,`secure_key` ) values ('$gender','1','$fname','$lname','$email','$pass','$last_pass_gen','$date_of_birth','1','$date_added','$date_updated','$s_key') ";
	Db::getInstance()->Execute($query);
...
function popUpWindow($msg='',$data=array()){
	...
	?>
	<link rel="stylesheet" type="text/css" href="<?php echo __PS_BASE_URI__;?>/modules/sociallogin/sociallogin_style.css" />
...
function popup_verify($msg, $social_id='') {
	...
	$html ='
	<link rel="stylesheet" type="text/css" href="'.__PS_BASE_URI__.'/modules/sociallogin/sociallogin_style.css" />
	<div id="fade" class="LoginRadius_overlay">
	<div id="popupouter">
	<div id="popupinner">
	...
	$html.='
	<input type="button" value="'.$module->all_messages('Ok').'" onclick=window.location.assign("'.$home.'") class="inputbutton" />';
	if(!empty($social_id)) {
		$html.='
