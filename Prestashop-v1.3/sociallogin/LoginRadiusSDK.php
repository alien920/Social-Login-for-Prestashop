...
class LoginRadius
{
	...
	public function loginRadiusGetData($api_secrete)
	{
		...
		if (Tools::getValue('token'))
		{
			...
			$user_profile = json_decode($json_response);
	...
	public function loginRadiusCallApi($validate_url)
	{
		...
		{
			$json_response = file_get_contents($validate_url);
			if (isset($http_response_header) && strpos($http_response_header[0], '400') !== false
