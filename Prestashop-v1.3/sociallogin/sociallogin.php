...
class SocialLogin extends Module
{
	public function __construct()
	{
		...
		$this->description = $this->l('Let your users log in and comment via their accounts with popular ID providers
		such as Facebook, Google, Twitter, Yahoo, Vkontakte and over 25 more!.');
	}
	...
	*/
	public function hookLeftColumn($params, $str = '')
	{
		...
		{
			if (!preg_match('/^\{?[A-Z0-9]{8}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{12}\}?$/i', $loginradius_api_secret)
	...
	*/
	public function hookHome($params)
	{
		...
		if (Configuration::get('enable_social_sharing') == '0')
		{
			...
			|| Configuration::get('social_share_cart') == '1')
			{
				if (Configuration::get('enable_social_horizontal_sharing') == '0')
				{
	...
	*/
	public function hookTop()
	{
		...
		if ($cookie->isLogged())
		{
			...
			if (Tools::getValue('token') && !empty($userprofile))
				linking($cookie, $userprofile);
			//Remove account linking when user click on remove button.
			if (Tools::getValue('id_provider'))
			{
				$module = new sociallogin();
				$getdata = Db::getInstance()->ExecuteS('SELECT * FROM '.pSQL(_DB_PREFIX_.'customer').' as c
				WHERE c.email='." '$cookie->email' ".' LIMIT 0,1');
				$num = (!empty($getdata['0']['id_customer']) ? $getdata['0']['id_customer'] : '');
				$deletequery = 'delete from '.pSQL(_DB_PREFIX_.'sociallogin')." where provider_id ='".Tools::getValue('id_provider')."'";
		...
		{
			global $cookie;
			if (Tools::getValue('LoginRadius') && Tools::getValue('LoginRadius') == 'Submit'
			&& (Tools::getValue('hidden_val') == $cookie->sl_hidden))
			{
				...
				if (Configuration::get('user_require_field') == '1')
				{
					...
					if ((empty($data['city']) && strpos($profilefield, '4') !== false) || empty($data['state'])
					|| (empty($data['phonenumber']) && strpos($profilefield, '5') !== false)
					|| (empty($data['address']) && strpos($profilefield, '6') !== false)
					|| (empty($data['zipcode']) && strpos($profilefield, '8') !== false)
					|| (empty($data['country']) && strpos($profilefield, '3') !== false)
					|| empty($data['email']) || (empty($data['addressalias']) && strpos($profilefield, '7') !== false)
				...
				//Validate email address is from email popup.
	...
	*/
	...
	{
		global $smarty;
		$smarty->assign('in_footer', true);
		return $this->display(__FILE__, 'my-account.tpl');
	}
	/*
	 * Show all message according to Language.
	 */
	public function allLangMessage($value)
	{
		return $this->l($value);
	}

	public function allConfigMessage()
	{
		$msg = '';
		$msg .= $this->l('This social ID is already linked with an account. Kindly unmap the current ID before linking
		new Social ID.');
		...
		$msg .= $this->l('Your confirmation link has been sent to your email address. Please verify your email by
	...
	*/
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitKeys'))
		{
			...
			elseif ((!empty($loginradius_api_key) || !empty($loginradius_api_secret))
			&& ($loginradius_api_key == $loginradius_api_secret))
			...
			Configuration::updateValue('profilefield',
			count(Tools::getValue('profilefield')) > 0 ? serialize(Tools::getValue('profilefield')) : '');
	...
	<img src="../img/t/AdminTranslations.gif" style="margin-right:5px">
	<a href="index.php?tab=AdminTranslations&token='.Tools::getAdminToken('AdminTranslations'.intval(33).intval($cookie->id_employee)).
	'&type=modules#prestashop_sociallogin_sociallogin">Social Login module Translation</a></div>
	<link href="'.__PS_BASE_URI__.'modules/sociallogin/css/socialloginandsocialshare.css" rel="stylesheet" type="text/css" media="all" />
	<script type="text/javascript" src="'.__PS_BASE_URI__.'modules/sociallogin/js/checkapi.js"></script>
	...
	<div style="float:left;">';
		...
		{
			$html .= '<div title="warning" style="background-color: #FFFFE0; border:1px solid #E6DB55;
			margin-bottom:5px; width: 900px;padding: 4px 2px 2px 30px;
			background-image: url(../modules/sociallogin/img/warning.png);
			background-repeat: no-repeat;background-position:left;">
			'.$this->l('To activate the Social Login, insert LoginRadius API Key and Secret in the API Settings section below.
			Social Sharing do not require API Key and Secret.').'</div>';
		}
		$html .= '<div style="float:left;width:70%;">
	<div>
	<fieldset class="sociallogin_form sociallogin_form_main" style="background: none repeat scroll 0 0 #FFFFE0;
	border: 1px solid #E6DB55;"><div class="row row_title" style="color: #000000; font-weight:normal; background:none;">
	<strong><h3>'.$this->l('Thank you for installing the LoginRadius Prestashop Extension!').'<h3></strong>
	</div>
	...
	If you do not have an account, click').'<a href="http://www.loginradius.com" target="_blank"> here </a>
	'.$this->l('and create one for FREE!').'
	</div>
	<div class="row" style="color: #000000; width:90%; line-height:160%; background:none;">
	'.$this->l('We also have Social Plugin for').'
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#joomla-extension" target="_blank">Joomla</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#wordpress-plugin" target="_blank">WordPress</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#drupal-module" target="_blank">Drupal</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#vbullentin-plugin" target="_blank">vBulletin</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#vanilla-add-ons" target="_blank">VanillaForum</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#magento-extension" target="_blank">Magento</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#oscommerce-add-ons" target="_blank">osCommerce</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#xcart-extension" target="_blank">X-Cart</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#zencart-plugin" target="_blank">Zen-Cart</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#dotnetnuke-module" target="_blank">DotNetNuke</a>,
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#phpbb" target="_blank">phpBB</a> and
	<a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms#smf" target="_blank">SMF</a>!
	</div>
	<div class="row row_button" style="background:none; border:none; background:none;">
	<div class="button2-left">
	<div class="blank" style="margin:0 0 10px 0;">
	<div class="button" style="float:left; cursor:pointer;">  <a class="modal" href="http://www.loginradius.com/" target="_blank">
	'.$this->l('Set up my FREE account!').'</a></div>
	</div>
	</div>
	</div>
	</fieldset>
	...
	<dt class="panel1 open" id="panel1"  style="cursor:pointer;" onclick=javascript:panelshow("first") ><span>
	'.$this->l('API Settings').'</span></dt>
	<dt class="panel2 closed" id="panel2" style="cursor:pointer;" onclick=javascript:panelshow("second") ><span>
	'.$this->l('Social Login').'</span></dt>
	<dt class="panel3 closed" id="panel3" style="cursor:pointer;" onclick=javascript:panelshow("third") ><span>
	'.$this->l('Social Share').'</span></dt>
	</dl>
	<div class="current">
	...
	'.$this->l('To activate the plugin, insert LoginRadius API Key & Secret').
	'<a href="http://support.loginradius.com/customer/portal/articles/677100-how-to-get-loginradius-api-key-and-secret"
	target="_blank" style="color: #0088CC;"> '.$this->l('(How to get it?)').' </a></span>
	<br/><br />
	'.$this->l('API Key').' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="text" size="50" name="API_KEY" id="API_KEY" value="'.$api_key.'" />
	<br /><br />
	'.$this->l('API Secret').'	&nbsp;&nbsp;
	...
	<tr class="row_white">
	<td colspan="2" ><span class="subhead">'.$this->l('What API Connection Method do you prefer to
	enable API communication?').'</span>
	<br /><br />
	<input type="radio" name="CURL_REQ" id="CURL_REQ" value="curl"
	'.(Configuration::get('CURL_REQ') == 'curl' ? 'checked="checked" ' : '').' />
	'.$this->l('Use cURL (Recommended API connection method but sometimes this is disabled at hosting server.)').'
	<br /><br />
	<input type="radio" name="CURL_REQ" id="FSOCKOPEN_REQ" value="fsockopen"
	'.(Configuration::get('CURL_REQ') == 'fsockopen' ? 'checked="checked" ' : '').'/>
	'.$this->l('Use FSOCKOPEN (Choose this option, if cURL is disabled at your hosting server.)').'
	</td>
	</tr>
	<tr class="row_white">
	<td>
	...
	<div class="button2-left">
	<div class="blank">
	<input type="button" class="button" name="verify_api_setting"  size="50" value="'.$this->l('Verify API Settings').'"
	onclick="MakeRequest()" style="cursor:pointer;"/>
	</a>
	</div>
	</div>
	</div>
	...
	<tr><td colspan="2" ><span class="subhead">'.$this->l('Do you want to enable Social Login for your website?')
	.'</span><br /><br />
	...
	<tr><td colspan="2" ><span class="subhead">'.$this->l('How many social icons would you like to be displayed per row?')
	.'</span><br /><br />
	<input type="text" name="social_login_icon_column" id="social_login_icon_column"  size="-7"
	value="'.$social_login_icon_column.'" /></td></tr>
	<tr class="row_white"><td colspan="2" ><span class="subhead">'.$this->l('What background color would you like to
	use for the Social Login interface?').'<a title="'.$this->l('Leave empty for transparent. You can enter hexa-decimal
code of the color as well as name of the color.').'" href="javascript:void(0)" style="text-decoration:none;color: #0088CC;">
	(?)</a></span><br /><br />
	<input type="text" name="social_login_background_color" id="social_login_background_color"  size="50"
	value="'.$social_login_background_color.'" /></td></tr>
	</table>
	<table class="form-table sociallogin_table">
	<tr>
	...
	<tr >
	<td colspan="2" ><span class="subhead">'.$this->l('Where do you want to redirect your users after successfully log in?').
	'</span><br /><br />
	<input name="LoginRadius_redirect" value="backpage" type="radio" onclick="javascript:hidetextbox(1);" '.$checked[0].' />
	'.$this->l('Redirect to Same page (Same as traditional login)').' <br/>
	<input name="LoginRadius_redirect" value="profile" type="radio" onclick="javascript:hidetextbox(1);" '.$checked[1].' />
	'.$this->l('Redirect to the profile').' <br/>
	<input name="LoginRadius_redirect" value="url" type="radio" onclick="javascript:hidetextbox(0);" '.$checked[2].' />
	'.$this->l('Redirect to the following url:').'<br/>
	<input style ="display:none;" type="text" name="redirecturl" id="redirecturl"  size="40" value="'.$redirect.'" />
	</td>
	</tr>	
	<tr class="row_white"> <td colspan="2" ><span class="subhead">'.$this->l('Please enter the Title to be shown on
	Social Login interface').'</span><br/><input type="text" name="TITLE"  size="50" value="'.$title.'" /></td></tr><tr >
	<td colspan="2" ><span class="subhead">'.$this->l('Do you want your existing user to automatically link to their
	social accounts in case their Prestashop account email address matches with their social account email ID?').'</span><br /><br />
	<input type="radio" name="ACC_MAP" value="0"
	'.(!Tools::getValue('ACC_MAP', Configuration::get('ACC_MAP')) ? 'checked="checked" ' : '').'/>
	'.$this->l('YES, link social accounts to Prestashop account').'<br /><br />
	<input type="radio" name="ACC_MAP" value="1"
	'.(Tools::getValue('ACC_MAP', Configuration::get('ACC_MAP')) ? 'checked="checked" ' : '').'/>
	'.$this->l('NO, I want my existing users to continue using native Prestashop login').'
	</td>
	</tr>
	<tr class="row_white">
	<td colspan="2" ><span class="subhead">'.$this->l('Do you want to send emails to admin after new
	User registrations at your website?').'</span><br /><br />
	<input type="radio" name="SEND_REQ" value="1"
	'.(Tools::getValue('SEND_REQ', Configuration::get('SEND_REQ')) ? 'checked="checked" ' : '').'/>
	'.$this->l('Yes').'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="radio" name="SEND_REQ" value="0"
	'.(!Tools::getValue('SEND_REQ', Configuration::get('SEND_REQ')) ? 'checked="checked" ' : '').'/> '.$this->l('No').'
	</td>
	</tr>
	<tr>
	<td colspan="2" ><span class="subhead">'.$this->l('Do you want to send emails to customer after new
	User registrations at your website?').'</span><br /><br />
	...
	(A pop-up will open asking user to
	provide details of the fields not coming from Social ID provider)').'</span><br /><br />
	...
	<tr>
	<td colspan="2" ><span class="subhead">'.$this->l('A few ID Providers do not supply users e-mail IDs as
	part of user profile data. Do you want users to provide their email IDs before completing registration process?')
			.'</span><br /><br />
	<input type="radio" name="EMAIL_REQ" value="0"
	'.(!Tools::getValue('EMAIL_REQ', Configuration::get('EMAIL_REQ')) ? 'checked="checked" ' : '').' />
	'.$this->l('YES, get real email IDs from the users (Ask users to enter their email IDs in a pop-up)').'<br /><br />
	<input type="radio" name="EMAIL_REQ" value="1"
	'.(Tools::getValue('EMAIL_REQ', Configuration::get('EMAIL_REQ')) ? 'checked="checked" ' : '').'/>
	'.$this->l('NO, just auto-generate random email IDs for the users').'
	</td>
	</tr>
	<tr >
	<input id="connection_url" type="hidden" value="" />
	</tr><tr class="row_white"><td>
	<span class="subhead">'.$this->l('Please enter the message to be displayed to the user in the pop-up').'</span>
	<br /><br /><input type="text" name="POPUP_TITLE"  size="50" value="'.(empty($popup_title) ? $this->l('Please fill
	the following details to complete the registration') : $popup_title).'" />
	<br /></td></tr>
	<tr>
	<td>
	<span class="subhead">Please enter the message to be shown to the user in case of an invalid entry in popup</span>
	<br /><br /><input type="text" name="ERROR_MESSAGE"  size="50" value="'.(empty($error_message) ? $this->l('Please enter
	the following fields') : $error_message).'" />
	</td>
	</tr>
	</table>
	<table class="form-table sociallogin_table">
	...
	<tr>
	<td colspan="2" ><span class="subhead">'.$this->l('Do you want to update User Profile Data in your
	Prestashop database, every time user logs into your website?').
	'<a title="'.$this->l('If you disable this option, user profile data will be saved only once
when user logs in first time at your website, user profile details will not be updated in your Prestashop database,
even if user changes his/her social account details.').'" href="javascript:void(0)" style="text-decoration:none">
	...
	<tr><td colspan="2" ><span class="subhead">'.$this->l('Do you want to enable Social Sharing for your website?').
	'</span><br /><br /><input type="radio" name="enable_social_sharing" value="0"
	...
	border-right:8px solid transparent; border-left:8px solid transparent; margin:-28px 0 0 2px"';
	$html .= '<a id="mymodal" href="javascript:void(0);" onclick="Makehorivisible();"><b>
	'.$this->l('Horizontal').'</b></a> &nbsp;|&nbsp;
	...
	<tr><td><span id = "arrow" '.$style_visible.'></span></td></tr>
	...
	'.$this->l('Do you want to enable Vertical Social Sharing for your website?').'</span><br /><br />
	...
	<td colspan="2" ><span class="subhead">'.$this->l('Enter the text that you wish to be displayed
	above the Social Sharing Interface. Leave the field blank if you dont want any text to be displayed.').'</span>
	...
	<label for="hori32"><input name="chooseshare" id = "hori32"
	'.(Configuration::get('chooseshare') == 0 || Configuration::get('chooseshare') == '' ? 'checked="checked"' : '').'
	type="radio"  value="0" style="margin: 2px 10px 0 0; display: block; float: left !important;"
	 onclick ="loginradius_horizontal_sharing();" /> <img src = "../modules/sociallogin/img/horizonSharing32.png"/>
	 <br /><br /></label>
	<label for="hori16"><input name="chooseshare" id = "hori16"
	'.(Configuration::get('chooseshare') == 1 ? 'checked="checked"' : '').'
	type="radio" value="1" style="margin: 2px 10px 0 0; display: block; float: left !important;"
	onclick ="loginradius_horizontal_sharing();" /> <img src = "../modules/sociallogin/img/horizonSharing16.png" />
	<br /><br /></label>
	<label for="horithemelarge"><input name="chooseshare" id = "horithemelarge"
	'.(Configuration::get('chooseshare') == 2 ? 'checked="checked"' : '').'  type="radio" value="2"
	style="margin: 2px 10px 0 0; display: block; float: left !important;" onclick ="loginradius_horizontal_simple();" />
	<img src = "../modules/sociallogin/img/single-image-theme-large.png" /><br /><br /></label>
	<label for="horithemesmall"><input name="chooseshare" id = "horithemesmall"
	'.(Configuration::get('chooseshare') == 3 ? 'checked="checked"' : '').'
	type="radio" value="3" style="margin: 2px 10px 0 0; display: block; float: left !important;"
	onclick ="loginradius_horizontal_simple();" /> <img src = "../modules/sociallogin/img/single-image-theme-small.png" />
	<br /><br /></label>
...
onclick ="loginradius_horizontal_hybrid();" /><img src = "../modules/sociallogin/img/hybrid-horizontal-horizontal.png" />
<br /><br /></label>
	<label for="hybrid-horizontal-vertical">
	<input name="chooseshare" id = "hybrid-horizontal-vertical"
	'.(Configuration::get('chooseshare') == 8 ? 'checked="checked"' : '').'
	type="radio" value="8" style="margin: 2px 10px 0 0; display: block; float: left !important;"
	onclick ="loginradius_horizontal_hybrid();" /> <img src = "../modules/sociallogin/img/hybrid-horizontal-vertical.png" /></label>
	</div>';
	$html .= '<div id="sharevertical" style=display:none>
	<label for="vertibox32"><input name="chooseverticalshare" id = "vertibox32"
	'.(Configuration::get('chooseverticalshare') == 4 || Configuration::get('chooseverticalshare') == '' ? 'checked="checked"' : '').'
	type="radio" value="4" style="vertical-align:top" onclick ="loginradius_vertical_sharing();"/>
	<img src =  "../modules/sociallogin/img/32VerticlewithBox.png" /></label>
	...
	onclick ="loginradius_vertical_sharing();"/> <img src = "../modules/sociallogin/img/16VerticlewithBox.png"
...
'.(Configuration::get('chooseverticalshare') == 7 ? 'checked="checked"' : '').'
type="radio" value="7" style="vertical-align:top" onclick ="loginradius_vertical_counter();"/>
<img src = "../modules/sociallogin/img/hybrid-verticle-horizontal.png" style="vertical-align:top"/></label>
	<br /><br />
	...
	<p style="margin:0 0 6px 0; padding:0px;color:#000000;"><strong>'.$this->l('Select the position of Social Sharing widget').'</strong></p>';
		$html .= '<input name="choosesharepos" id = "topleft" type="radio"
		'.(Configuration::get('choosesharepos') == 0 || Configuration::get('choosesharepos') == '' ? 'checked="checked"' : '').' value="0" />Top Left<br />
	<input name="choosesharepos" id = "topright" type="radio"
	'.(Configuration::get('choosesharepos') == 1 ? 'checked="checked"' : '').' value="1" />Top Right<br />
	<input name="choosesharepos" id = "bottomleft" type="radio"
	'.(Configuration::get('choosesharepos') == 2 ? 'checked="checked"' : '').' value="2" />Bottom Left<br />
	<input name="choosesharepos" id = "bottomright" type="radio"
	'.(Configuration::get('choosesharepos') == 3 ? 'checked="checked"' : '').' value="3" />Bottom Right<br /><br />
	<p style="margin:0 0 6px 0; padding:0px;color:#000000;"><strong> <strong>
	'.$this->l('Specify distance of vertical sharing interface from top. (Leave empty for default behaviour)').'</strong>
	<a title="'.$this->l('Enter a number (For example - 200). It will set the \'top\' CSS attribute of the interface to
	the value specified. Increase in the number pushes interface towards bottom.').'" href="javascript:void(0)"
	style="text-decoration:none"> (<span style="color:#3CF;">?</span>)</a></strong></p><input type="text" id="topoffset"
	name="verticalsharetopoffset" value="'.(is_numeric(Configuration::get('verticalsharetopoffset')) ?
	Configuration::get('verticalsharetopoffset').'px' : '').'" >
	</div></div></div></div>
	</td>
	</tr>
	...
	(All other sharing networks will be shown as part of LoginRadius sharing icon)').'</span><br />
	...
	<ul id="sortable" style="height:35px;">';
		...
		foreach ($rearrange_settings as $provider)
			$li .= '<li title="'.$provider.'" id="loginRadiusLI'.$provider.'"
			class="lrshare_iconsprite32 lrshare_'.$provider.'">
	...
	<ul id="verticalsortable" style="height:35px;">';
		...
		foreach ($vertical_rearrange_settings as $provider)
			$li .= '<li title="'.$provider.'" id="loginRadiusverticalLI'.$provider.'"
			class="lrshare_iconsprite32 lrshare_'.$provider.'">
	...
	<tr>
	<td>';
		if (Tools::getValue('social_share_home') == 0
		&& Tools::getValue('social_share_cart') == 0
		&& Tools::getValue('social_share_product') == 0)
			Configuration::updateValue('social_share_home', 1);
		$html .= '<input type="checkbox" name="social_share_home" value="1"
		'.(Tools::getValue('social_share_home', Configuration::get('social_share_home')) ? 'checked="checked"' : '').' />
		Show on Home page</td><td>
	<input type="checkbox" name="social_share_cart" value="1"
	'.(Tools::getValue('social_share_cart', Configuration::get('social_share_cart')) ? 'checked="checked"' : '').' />
	Show on Cart page</td><td>
	<input type="checkbox" name="social_share_product" value="1"
	'.(Tools::getValue('social_share_product', Configuration::get('social_share_product')) ? 'checked="checked"' : '').' />
	...
	<tr>
	<td>';
		if (Tools::getValue('social_verticalshare_home') == 0
		&& Tools::getValue('social_verticalshare_cart') == 0
		...
		Show on Home page</td><td>
	<input type="checkbox" name="social_verticalshare_cart" value="1"
	'.(Tools::getValue('social_verticalshare_cart', Configuration::get('social_verticalshare_cart')) ? 'checked="checked"' : '').' />
	Show on Cart page</td><td>
	<input type="checkbox" name="social_verticalshare_product" value="1"
	...
	<h3 style="border-bottom:#000000 1px solid; margin:0px; padding:0 0 6px 0;
	border-bottom: 1px solid #B3E2FF; color: #000000; margin:10px;">Help & Documentations</h3>
	<ul class="help_ul">
	<li>
	<a href="http://support.loginradius.com/customer/portal/articles/1107175-implementation-of-social-login-on-prestashop-v1-5-website"
	target="_blank">Plugin Installation, Configuration and Troubleshooting</a></li>
	<li><a href="http://support.loginradius.com/customer/portal/articles/677100-how-to-get-loginradius-api-key-and-secret"
	 target="_blank">How to get LoginRadius API Key & Secret</a></li>
	<li><a href="http://support.loginradius.com" target="_blank">Support Documentations</a></li>
	<li><a href="http://community.loginradius.com/" target="_blank">Discussion Forum</a></li>
	<li><a href="https://www.loginradius.com/loginradius/what-is-loginradius" target="_blank">About LoginRadius</a></li>
	<li><a href="https://www.loginradius.com/loginradius/product-overview" target="_blank">LoginRadius Products</a></li>
	<li><a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-cms" target="_blank">Social Plugins</a></li>
	<li><a href="https://www.loginradius.com/loginradius-for-developers/loginRadius-sdks" target="_blank">Social SDKs</a></li>
	</ul>
	</div>
	<div style="clear:both;"></div>
	<div style="background:#EAF7FF; border: 1px solid #B3E2FF;  margin:0 0 10px 0; overflow:auto;">
	<h3 style="border-bottom:#000000 1px solid; margin:0px; padding:0 0 6px 0;
	border-bottom: 1px solid #B3E2FF; color: #000000; margin:10px;">Stay Update!</h3>
	...
	<ul class="stay_ul">
	<li class="first">
	<iframe rel="tooltip" scrolling="no" frameborder="0" allowtransparency="true" style="border: none;
	overflow: hidden; width: 46px; height: 63px;"
	src="//www.facebook.com/plugins/like.php?app_id=194112853990900&amp;
	href=http%3A%2F%2Fwww.facebook.com%2Fpages%2FLoginRadius%2F119745918110130&amp;send=false&amp;layout=box_count&amp;
	width=90&amp;show_faces=false&amp;action=like&amp;colorscheme=light&amp;font=arial&amp;height=9"
	data-original-title="Like us on Facebook"></iframe>
	</li>
	</ul>
	<div>
	</div>
	...
	public static function jsInterface()
	{
		...
		WHERE c.email='." '$cookie->email' ".' LIMIT 0,1');
		$num = (!empty($getdata['0']['id_customer']) ? $getdata['0']['id_customer'] : '');
		$linkedprovider = Db::getInstance()->ExecuteS('SELECT * from '.pSQL(_DB_PREFIX_.'sociallogin')."
		where `id_customer`='".$num."'");
