...
function linking($arrdata, $user_profile)
{
	...
	if (!empty($user_profile))
	{
		...
		$sql = "SELECT COUNT(*) as num from $tbl where `id_customer`='".$num."' and `Provider_name`='".$user_profile->Provider."'";
		...
		if ($row['num'] == 0)
		{
			...
			AS sl ON sl.id_customer=c.id_customer WHERE sl.provider_id= "'.$user_profile->ID.'"');
			if (empty($check_user_id['0']['id_customer']))
				Db::getInstance()->Execute('DELETE FROM '.$tbl."  WHERE provider_id='".$user_profile->ID."'");
			$lr_id = Db::getInstance()->ExecuteS('SELECT provider_id FROM '.$tbl."  WHERE provider_id= '".$user_profile->ID."'");
			...
			{
				$query = "INSERT into $tbl (`id_customer`,`provider_id`,`Provider_name`,`verified`,`rand`)
				values ('$num','".$user_profile->ID."' , '".$user_profile->Provider."','1','') ";
				Db::getInstance()->Execute($query);
		...
		//Show Warning message.
			...
			$cookie->lrmessage = $module->allLangMessage('This social ID is already linked with an account. Kindly unmap the current ID before
			linking new Social ID.');
	}
...
function loginRadiusVerifiedUserLogin($user_id, $lrdata, $td_user = '')
{
	...
	} else
	{
		...
		$msg = $module->allLangMessage('Your confirmation link has been sent to your email address. Please verify your email by clicking on
		confirmation link.');
...
{
	$share_script = '<script type="text/javascript">var islrsharing = true; var islrsocialcounter = true;var hybridsharing = true;</script>
...
Tools::addCSS(__PS_BASE_URI__.'modules/sociallogin/css/sociallogin_style.css');
?>
...
if ($msg == '')
{
	...
	echo $msg = (!empty($show_msg) ? $show_msg : $module->allLangMessage('Please fill the following details to complete the registration'));
}
else
	echo $msg;
?>
	...
	} else
	{
		...
		if (is_array($states))
		{
			...
			$html .= '<div id="location-state-div" '.$style.'>
				<span class="spantxt">'.$module->allLangMessage('State').'</span> <select id="location-state" name="location-state" class="inputtxt">';
			if (empty($states))
...
if (Configuration::get('user_require_field') == '1')
{
	...
	{
		$html .= '<div>
			<span class="spantxt">'.$module->allLangMessage('City').'</span><input type="text" name="SL_CITY" id="SL_CITY" placeholder="City"
	...
	{
		$html .= '<div><span class="spantxt">'.$module->allLangMessage('Address Title').'</span><input type="text" name="SL_ADDRESS_ALIAS"
...
$html .= '<div><input type="hidden" name="hidden_val" value="'.$cookie->sl_hidden.'" />
	...
	<input type="button" value="'.$module->allLangMessage('Cancel').'" class="inputbutton" onclick="window.location.href=window.location.href;" />
	</div></div>
	</form>
	</div>
	</div>
	...
	if (!$cookie->isLogged())
	{
		...
		if (!empty($query['0']['id_customer']))
		{
			...
			$error_msg = "<p style ='color:red;'>".$error_message.'</p><p
								style="color: red;margin-bottom: -20px; font-size: 10px;">Email-adddress has already used.</p>';
			$lrdata['email'] = '';
...
function popupVerify($msg, $social_id = '')
{
	...
	$html .= '
	<input type="button" value="'.$module->allLangMessage('Ok').'" onclick="window.location.href=window.location.href;" class="inputbutton" />';
	if (!empty($social_id))
	{
		$html .= '
		<input type="hidden" value="'.$social_id.'" name="social_id_value" class="inputbutton" />
		<input type="submit" value="'.$module->allLangMessage('Resend Email Verification').'" name="resend_email_verification" class="inputbutton" />';
	}
...
{
	$lrdata['fullname'] = (!empty($user_profile->FullName) ? $user_profile->FullName : '');
	$lrdata['profilename'] = (!empty($user_profile->ProfileName) ? $user_profile->ProfileName : '');
	$lrdata['nickname'] = (!empty($user_profile->NickName) ? $user_profile->NickName : '');
	$lrdata['fname'] = (!empty($user_profile->FirstName) ? $user_profile->FirstName : '');
	$lrdata['lname'] = (!empty($user_profile->LastName) ? $user_profile->LastName : '');
